...
/**
 * Generates a professional PDF invoice for a service request.
...
export async function generateInvoicePDF(data) {
    return new Promise((resolve, reject) => {
        try {
            ...
            const fileName = `invoice_${data.invoiceId || Date.now()}.pdf`;
            ...
            doc.text(data.amount.toFixed(2), amountX, rowY, { align: 'right', width: 90 });

            // Platform Fee
            const rowY2 = rowY + 20;
            doc.text('Fee', itemX + 5, rowY2);
            doc.text('Platform & Convenience Fee', descX, rowY2);
            doc.text(data.platformFee.toFixed(2), amountX, rowY2, { align: 'right', width: 90 });
            ...
            doc.text(`₹${data.totalAmount.toFixed(2)}`, amountX, totalY, { align: 'right', width: 90 });
